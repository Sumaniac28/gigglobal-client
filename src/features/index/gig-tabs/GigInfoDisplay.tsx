            ...
            <main className="max-w-8xl container mx-auto mt-8">
                  ...
                  <>
                    ...
                    <div className="flex w-full gap-x-1 self-center">
                      ...
                      <div className="ml-2 mt-[2px] flex gap-1 text-sm">
                        <span className="text-yellow-400">{rating(gig.current.ratingSum / gig.current.ratingsCount)}</span>
              ...
              <div className="flex flex-wrap">
                <div className="order-last flex w-full flex-col p-4 lg:order-first lg:w-2/3">
                  <div className="relative flex max-h-[600px] cursor-pointer justify-center bg-[#F9FAFB]">
                    {!isLoading && isSuccess && (
                      <img
                    ...
                    {isLoading && !isSuccess && (
                      <div className="object-contains flex h-[600px] w-full transition-all duration-500 hover:scale-105">
                        <FaCircleNotch className="animate-spin h-10 w-full mr-3 flex self-center" size={40} color="#14B8A6" />
                      </div>
                    )}
                  </div>

                  <div className="mt-10 pb-6 text-lg font-semibold text-[#111111]">About This Gig</div>
                  <div className="pb-6 text-[#4B5563]">
                  ...
                  <div className="grid grid-cols-1 gap-y-4 lg:grid-cols-2">
                    <div className="flex flex-col">
                      ...
                      <span className="font-normal text-[#111111]">{gig.current.categories}</span>
                    </div>
                    <div className="flex flex-col">
                        ...
                        {gig.current.subCategories &&
                          gig.current.subCategories.map((category: string, index: number) => (
                            <span className="font-normal" key={uuidv4()}>
                              {`${category}${index !== gig.current.subCategories.length - 1 ? ',' : ''}`}
                            </span>
                          ))}
                      </div>
                    </div>
                ...
                <div className="w-full p-4 lg:w-1/3">
                  <StickyBox>
                    <div className="mb-8 border border-[#E5E7EB]">
                      <div className="flex border-b border-[#E5E7EB] px-4 py-2">
                        <h4 className="font-bold text-[#14B8A6]">${gig.current.price}</h4>
                      </div>
                      <ul className="mb-0 list-none px-4 py-2">
                        <li className="flex justify-between">
                          <div className="ml-15 flex w-full pb-3">
                            <div className="text-base font-bold text-[#111111]">{gig.current.basicTitle}</div>
                          </div>
                        </li>
                        <li className="flex justify-between">
                          <div className="ml-15 flex w-full pb-4">
                            <div className="text-sm font-normal text-[#4B5563]">{gig.current.basicDescription}</div>
                          </div>
                        </li>
                        <li className="flex justify-between">
                          <div className="ml-15 flex w-full pb-3 text-[#111111]">
                        ...
                        <li className="flex justify-between">
                          <div className="ml-15 flex w-full py-1">
                            <Button
                              className="text-md flex w-full cursor-pointer justify-between rounded bg-[#14B8A6] px-8 py-2 font-bold text-white hover:bg-[#0F766E] focus:outline-none"
