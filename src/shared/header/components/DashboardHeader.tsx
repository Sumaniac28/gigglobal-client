...
const DashboardHeader: FC = (): ReactElement => {
  ...
  return (
    <>
      ...
      <header>
        <nav className="navbar peer-checked:navbar-active relative z-20 w-full border-b border-[#1F2937] bg-black shadow-2xl shadow-gray-900/5 backdrop-blur">
          <div className="m-auto px-6 xl:container md:px-12 lg:px-6">
            <div className="flex flex-wrap items-center justify-between gap-6 md:gap-0 md:py-3 lg:py-5">
                ...
                <div className="flex w-full">
                  <label htmlFor="hbr" className="peer-checked:hamburger relative z-20 -ml-4 block cursor-pointer p-6 lg:hidden">
                    <Button
                      ...
                      label={
                        <>{openSidebar ? <FaTimes className="h-6 w-6 text-teal-500" /> : <FaBars className="h-6 w-6 text-teal-500" />}</>
                      }
                    />
                  </label>
                  <div className="w-full gap-x-4 md:flex">
                    <Link
                      ...
                      className="relative z-10 flex cursor-pointer justify-center self-center text-2xl font-themeFont font-semibold text-white lg:text-3xl"
                    >
                      GigGlobal
                    </Link>
                  </div>
              ...
              <div className="navmenu mb-16 hidden w-full cursor-pointer flex-wrap items-center justify-end space-y-8 rounded-3xl border border-[#374151] bg-black p-6 shadow-2xl shadow-gray-800/20 md:flex-nowrap lg:m-0 lg:flex lg:w-6/12 lg:space-y-0 lg:border-0 lg:bg-transparent lg:p-0 lg:shadow-none">
                <div className="text-[#9CA3AF] lg:pr-4">
                  <ul className="flex text-base font-medium">
                    <li className="relative flex items-center">
                      <Link
                        to={`/${lowerCase(`${seller?.username}`)}/${seller?._id}/manage_orders`}
                        className="px-3 text-white hover:text-teal-400"
                      >
                        <span>Orders</span>
                      </Link>
                    </li>
                        ...
                        label={
                          <>
                            ...
                            {authUsername === authUser.username && (
                              <span className="absolute bottom-1 left-8 h-2.5 w-2.5 rounded-full border-2 border-black bg-teal-400"></span>
                            )}
                          </>
                        }
                      />
