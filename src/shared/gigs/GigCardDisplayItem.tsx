      ...
      <div className="mb-8 flex cursor-pointer flex-col gap-2">
        ...
        <div className="flex items-center gap-2 relative">
          ...
          {sellerUsername.current === gig.username && (
            <span className="bottom-0 left-5 absolute w-2.5 h-2.5 bg-green-400 border-2 border-[#F9FAFB] rounded-full"></span>
          )}
          <div className="flex w-full justify-between">
            <span className="text-md hover:underline text-[#111111]">
              {linkTarget ? (
                ...
                >
                  <strong className="text-sm font-medium md:text-base text-[#111111]">{gig.username}</strong>
                </Link>
              ) : (
                <strong className="text-sm font-medium md:text-base text-[#111111]">{gig.username}</strong>
              )}
            ...
            {showEditIcon && (
              <FaPencilAlt
                className="mr-2 flex self-center text-[#4B5563] hover:text-[#0F766E]"
                size={15}
        ...
        <div>
          <Link to={`/gig/${lowerCase(`${gig.username}`)}/${title}/${gig.sellerId}/${gig.id}/view`} onClick={() => saveGigTitle(gig)}>
            <p className="line-clamp-2 text-sm text-[#4B5563] hover:text-[#0F766E] md:text-base">{gig.basicDescription}</p>
          </Link>
        </div>
        ...
        <div>
          <strong className="text-sm font-bold md:text-base text-[#14B8A6]">From ${gig.price}</strong>
        </div>
      </div>
    </div>
  );
