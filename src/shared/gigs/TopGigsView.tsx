    ...
    <div className="mx-auto my-8 flex flex-col overflow-hidden rounded-lg">
        ...
        <div className="flex items-start py-6">
          <div className="flex w-full flex-col justify-between">
            <div className="flex gap-2">
              <h2 className="text-base font-bold text-[#111111] md:text-lg lg:text-2xl">{title}</h2>
              {category && (
                <span className="flex self-center text-base font-bold cursor-pointer text-[#14B8A6] hover:text-[#0F766E] hover:underline md:text-lg lg:text-2xl">
            ...
            <h4 className="pt-1 text-left text-sm text-[#4B5563]">{subTitle}</h4>
          </div>
        </div>
      )}
      ...
      <div className="m-auto flex h-96 w-full overflow-x-auto" ref={navElement}>
        {scroll.start && gigs.length > 2 && (
          <span
            onClick={slideLeft}
            className="absolute left-2 z-50 flex cursor-pointer justify-start self-center rounded-full bg-[#14B8A6] sm:left-3 md:left-7 lg:left-0"
          >
        ...
        {!scroll.end && gigs.length > 2 && (
          <span
            onClick={slideRight}
            className="absolute right-2 flex max-w-4xl cursor-pointer justify-end self-center rounded-full bg-[#14B8A6] sm:right-3 md:right-7 lg:right-0"
          >
