    ...
    <>
      ...
      <div className="relative">
        {gigCardItemModal.overlay && (
          <div className="absolute bottom-0 top-0 mb-8 w-full cursor-pointer border border-[#E5E7EB] bg-white">
            <div
              onClick={() => setGigCardItemModal({ ...gigCardItemModal, overlay: false })}
              className="absolute -right-[12px] -top-[12px] flex h-8 w-8 cursor-pointer items-center justify-center rounded-full border border-[#14B8A6] bg-white text-sm font-bold leading-[0] text-[#14B8A6]"
            >
              X
            </div>
            <ul className="list-none pl-0">
              <li>
                <div
                  ...
                  className="my-1 flex w-full cursor-pointer gap-4 px-4 pt-3 text-[#111111] hover:text-[#0F766E]"
                >
        ...
        <div className="mb-8 flex cursor-pointer flex-col gap-2 border border-[#E5E7EB]">
          <Link
          ...
          <div className="px-2">
            <Link
              ...
              to={`/gig/${lowerCase(`${gig.username}`)}/${title}/${gig.sellerId}/${gig.id}/view`}
            >
              <p className="line-clamp-2 text-sm text-[#4B5563] hover:text-[#0F766E]">{gig.basicDescription}</p>
            </Link>
          </div>
          <div className="flex gap-2 px-2 text-yellow-400">
            {parseInt(`${gig.ratingsCount}`) > 0 ? <FaStar className="mt-1" /> : <FaRegStar className="mt-1" />}(
            <span className="text-[#111111]">{rating(parseInt(`${gig.ratingSum}`) / parseInt(`${gig.ratingsCount}`))}</span>)
          </div>
          <div className="flex justify-between px-2 pb-2">
            <FaEllipsisH
              size={14}
              className="self-center text-[#4B5563] hover:text-[#0F766E]"
              onClick={() => setGigCardItemModal({ ...gigCardItemModal, overlay: true })}
            />
            <strong className="text-base font-normal text-[#14B8A6]">${gig.price}</strong>
          </div>
        </div>
      </div>
    </>
